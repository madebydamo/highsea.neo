    ...
    with {inherit (lib.neo) mkOption mkEnableOption;}; {
      options.neo.services.declarr = mkOption {
        type = types.submodule {
          options =
            {
              enabled = mkEnableOption "declarr service (auto-config for sonarr/radarr/prowlarr/qbittorrent/jellyseerr etc)" {rank = 0;};
              stateDir = mkOption {
                type = types.str;
            ...
            // lib.neo.mkServiceMeta {
              ...
              description = ''
                declarr provides declarative configuration syncing for the *arr stack (sonarr, radarr, prowlarr, and jellyseerr).
                In high_sea it is used to automatically link services together: qbittorrent is wired as the download client for sonarr/radarr/prowlarr, prowlarr registers the *arr apps, and jellyseerr (seerr) gets connections to jellyfin + *arr apps. All using stable API keys and container DNS names.
